...
// import KioskTable from 'src/components/KioskTable'
...
import { baseURL, KIOSKS_LIST } from '../config'
...
function Kiosks() {
  ...
  const [kiosksListLoading, setKiosksListLoading] = useState(true)
  ...
  const infiniteScroll = () => {
    ...
    if (scrollBoxID > scrollChildID && nextPage !== null && !kiosksListLoading) {
      getKiosksList(nextPage)
    }
  }

  const getKiosksList = (url) => {
    setKiosksListLoading(true)
    setMessageClass('message-hidden')
    axios
      .get(url, header)
      .then((response) => {
        setKiosksListLoading(false)
        const { results, next } = response?.data
        setNextPage(next)
        setKiosksList((prevState) => prevState.concat(results))
      })
      .catch((error) => {
        ...
        <div className="pb-8">
          <div className="flex items-center">
            ...
            >
              ...
              <input placeholder="جستجو در کیوسک" className="px-2 w-full align-right" />
          ...
          <div className="mt-8" id="scrollChildID">
            <KioskTable data={kiosksList} loading={kiosksListLoading} />
          </div>
        </div>
        <MessageBox
