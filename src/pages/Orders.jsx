...
import { baseURL, ORDER_LIST } from '../config'
...
function Orders() {
  ...
  const infiniteScroll = () => {
    ...
    if (scrollBoxID > scrollChildID && nextPage !== null && !orderListLoading) {
  ...
  const getOrderList = (url) => {
    ...
    axios
      ...
      .then((response) => {
        ...
        setNextPage(next)
        setOrderList((prevState) => prevState.concat(results))
      })
      .catch((error) => {
  ...
  const searchHandle = (e) => {
    ...
    setDelayTimer(
      setTimeout(function () {
        if (token) {
          const url = `${baseURL}${ORDER_LIST}?search=${value}`
          getOrderList(url)
        }
      }, 300)
    )
      ...
      <div className="h-full overflow-y-scroll p-5" ref={scrollBoxRef} id="scrollBoxID">
        <div className="pb-8">
          ...
          <div className="mt-8 mb-32" id="scrollChildID">
            <OrderTable data={orderList} loading={orderListLoading} />
          </div>
        </div>
        <MessageBox
